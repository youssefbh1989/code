...
class VendorModel extends Equatable implements EntityModel{
  int id;
  String name;
  String cover;
  String nameAr;
  ...
  factory VendorModel.fromJson(Map<String, dynamic> json) {
    ...
    return json == null
      ...
      total_user_rated: json["total_user_rated"].toString(),
      ...
      reviewedUsers: List.from(
          json["reviewed_users"] ?? []).map((x) => ReviewedUser.fromJson(x)).toList(),
      gallery: List.from(
          json["gallery"]??[]).map((x) => x['image']).toList().cast<String>(),
      categories: List.from(
          json["categories"]??[]).map((x) => CategoryModel.fromJson(x)).toList(),
    );
  }
  ...
  @override
  EntityModel clone() {
    return VendorModel.fromJson(toJson());
  }
