...
class SalonModel extends Equatable implements EntityModel {
  int id;
  String name;
  String cover;
  String nameAr;
  ...
  factory SalonModel.fromJson(Map<String, dynamic> json) {
    return json == null
      ...
      categories: List.from(json["categories"]??[]).map((x) => CategoryModel.fromJson(x)).toList(),
      gallery: List.from(
          json["gallery"]??[]).map((x) => x['image'].toString()).toList(),
    );
  }
  ...
  Map<String, dynamic> _toRequest() {
    return {
      ...
      "gallery": List.from(gallery.map((x) => {'image': x})),
    };
  }

  @override
  EntityModel clone() {
    return SalonModel.fromJson(toJson());
  }
